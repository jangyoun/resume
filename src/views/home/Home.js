...
function Home() {
  const [popupCompany, setPopupCompany] = useState({ visible: false, current: 0 });
  const [popupProject, setPopupProject] = useState({ visible: false, current: 0 });

  return (
    <div>
      <div style={{ width: "100%", height: 200, backgroundColor: "#999", marginBottom: -50, position: "relative" }}>
        {/* <img style={{ height: "100%", width: "100%", objectFit: "cover", position: "absolute" }} src={process.env.PUBLIC_URL + "/images/a_profile_bg.jpg"} /> */}
      ...
      <div style={{ width: "100%", display: "flex" }}>
        <div style={{ width: 80, height: 80, margin: "0px 10px 0px 20px" }}>
          <Image
            ...
            src={process.env.PUBLIC_URL + "/images/a_profile_thumb.png"}
          />
        </div>
      ...
      <div style={{ margin: "30px 20px 0px 20px" }}>
        ...
        {/* Company ========================================================================== */}
        <PopupCompany visible={popupCompany.visible} current={popupCompany.current} />
        <div>
          <div style={{ fontSize: 18, fontWeight: "bold", marginTop: 30 }}>Company</div>
          <Table
            size={"small"}
            dataSource={CONST.COMPANY_ARR}
            rowKey={(item) => item.id}
            scroll={{ x: 640 }}
            pagination={false}
            onRow={(item, idx) => {
              return {
                onClick: (event) => {
                  setPopupCompany({ visible: true, current: idx });
                },
              };
            }}
          >
