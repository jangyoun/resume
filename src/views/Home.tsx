            ...
            <div style={{ width: '100%', height: '100%', overflow: 'auto' }}>
                ...
                <div style={{ margin: "30px 20px 0px 20px" }}>
                    <Select
                        ...
                        defaultValue={'en'}
                        onChange={(value: string) => setEn(value === 'en')}
                    >
                        <Select.Option value="en" label="en"> 🇺🇸 English Resume </Select.Option>
                        <Select.Option value="kr" label="kr"> 🇰🇷 Korean Resume</Select.Option>
                    </Select>
                    <div style={{ fontSize: 18, fontWeight: "bold" }}>Full-Stack Developer</div>
                    <div style={{ fontSize: 18, margin: "0px 10px 30px 0px" }}>
                        {isEn
                            ? 'Have developed data crawling search system from a portal service company and DLNA, Miracast technology from a mobile phone manufacturer. Also have founded and managed a company about creating animation stickers and the company was sold to OGQ and I participated in projects related to AI and deep learning at OGQ. Currently, I am developing non-contact construction management platform using bluetooth device as a CTO at Buildprop.'
                            : '국내 포털사에서 검색 수집 머신 개발을, 국내 제조사에서 멀티미디어 미러링 개발 하였습니다. 우연한 기회로 콘텐츠 스타트업을 창업하였고, 그 과정에서 기술력을 인정받아 인수 합병을 경험하였습니다. 인수된 회사에서 신사업 및 A.I. 머신러닝 업무를 진행하였으며, 현재는 다수의 건설사 대표님들과 자산과 인력을 추적하는 솔루션을 개발 적용중에 있습니다. 전 개발 영역에 참여 하였으며, 개발 외적으로도 기획 / 디자인 / 운영 분들과 원만한 커뮤니케이션을 통해 다수의 프로젝트를 수행하였습니다.'}
                    </div>
                    ...
                    <div>
                        ...
                        <Table
                            size={"small"}
                            dataSource={isEn ? INFO.EDU_ARR_EN : INFO.EDU_ARR_KR}
                            rowKey={(item) => item.id}
                            scroll={{ x: 600 }}
                            pagination={false}
                        >
                    ...
                    <div>
                        ...
                        >
                            ...
                            <Table.Column title={isEn ? 'From ~ To' : '기간'} width={160} render={(item) => (<div> {item.from.format("YY.MM")} ~ {item.now ? "" : item.to.format("YY.MM")} </div>)} />
                    ...
                    <div>
                        ...
                        >
                            ...
                            />
                            <Table.Column title={isEn ? 'From ~ To' : '기간'} width={130} render={(item) => <div>{item.from.format("YY.MM")} ~ {item.now ? "" : item.to.format("YY.MM")}</div>} />
                            <Table.Column title={isEn ? 'MM' : 'MM'} width={60} render={(item) => <div>{item.to.diff(item.from, "months")}M</div>} />
                            <Table.Column title={isEn ? 'TAG' : '상세'} render={(item) =>
                    ...
                    <div>
                        ...
                        >
                            ...
                            />
                            <Table.Column
                                title={isEn ? 'Organizer' : '주최'} width={130} render={(item) => <div>{item.organizer}</div>} />
